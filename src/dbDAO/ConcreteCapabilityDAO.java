...
public class ConcreteCapabilityDAO {
	...
	public  List<ConcreteCapability>  getAllConcreteCapabilityByAbstractUserDev( AbstractCapability abstractCapability,User user)
	{
		  SessionFactory sessionFactory = HibernateUtil.getSessionFactory();
		  Session session = sessionFactory.openSession();
		  session.beginTransaction();
		  Query query = session.createQuery("from ConcreteCapability where abstractCapability= :abstractCapability and user= :user ");
		  query.setParameter("abstractCapability", abstractCapability);
	...
	public  List<ConcreteCapability>  getAllConcreteCapabilityByDomain( Domain domain)
	{
		  ...
		  Session session = sessionFactory.openSession();
		  session.beginTransaction();
		  Query query = session.createQuery("select cc from ConcreteCapability as cc ,AbstractCapability as ac  where cc.abstractCapability=ac.idAbstratCapability and ac.domain= :domain");
	...
	public  List<ConcreteCapability>  getAllConcreteCapabilityByDomainUser( Domain domain,User user)
	{
		  ...
		  Session session = sessionFactory.openSession();
		  session.beginTransaction();
		  Query query = session.createQuery("select cc from ConcreteCapability as cc ,AbstractCapability as ac  where cc.abstractCapability=ac.idAbstratCapability and ac.domain= :domain and cc.user= :user");
		  query.setParameter("domain", domain);
		  query.setParameter("user", user);
		  System.out.println("QUERY -->"+query.toString());
		  List<ConcreteCapability> concreteCapability = query.list();
	...
	public  ConcreteCapability  getConcreteCapabilityByID(Integer idConcreteCapability)
	{
		  ...
		  Session session = sessionFactory.openSession();
		  session.beginTransaction();
		  ConcreteCapability concreteCapabilityDB = (ConcreteCapability)session.get(ConcreteCapability.class,idConcreteCapability);
		  session.getTransaction().commit();
		  sessionFactory.close();
