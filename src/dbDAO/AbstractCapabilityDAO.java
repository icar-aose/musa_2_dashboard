...
public class AbstractCapabilityDAO {
	...
	public AbstractCapability getAbstractCapabilityByID(Integer idAbstractCapability) {
		...
		Session session = sessionFactory.openSession();
		session.beginTransaction();
		AbstractCapability abstractCapabilityDB = (AbstractCapability) session.get(AbstractCapability.class,
				idAbstractCapability);
		session.getTransaction().commit();
		sessionFactory.close();
	...
	public void deleteAbstractCapability(int idAbstractCapability) {
		...
		Session session = sessionFactory.openSession();
		session.beginTransaction();
		AbstractCapability abstractCapability = (AbstractCapability) session.load(AbstractCapability.class,
				idAbstractCapability);
