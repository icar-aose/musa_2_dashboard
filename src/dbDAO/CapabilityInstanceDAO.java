...
public class CapabilityInstanceDAO {
	 ...
	 public List<CapabilityInstance> getAllCapabilityInstanceByConcreteCapability(ConcreteCapability concreteCapability){
		 SessionFactory sessionFactory = HibernateUtil.getSessionFactory();
		  Session session = sessionFactory.openSession();
		  session.beginTransaction();
	 ...
	 public  CapabilityInstance  getCapabilityInstanceByID(Integer idCapabilityInstance)
		{
			  ...
			  Session session = sessionFactory.openSession();
			  session.beginTransaction();
			  CapabilityInstance capabilityInstanceDB = (CapabilityInstance)session.get(CapabilityInstance.class,idCapabilityInstance);
			  session.getTransaction().commit();
			  sessionFactory.close();
