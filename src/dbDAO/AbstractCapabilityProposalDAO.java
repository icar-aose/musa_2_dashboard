...
public class AbstractCapabilityProposalDAO {
	...
	public  AbstractCapabilityProposal getAbstractCapabilityProposalByID(Integer idAbstractCapabilityProposal)
	{
		  ...
		  Session session = sessionFactory.openSession();
		  session.beginTransaction();
		  AbstractCapabilityProposal abstractCapabilityProposalDB = (AbstractCapabilityProposal)session.get(AbstractCapabilityProposal.class,idAbstractCapabilityProposal);
		  session.getTransaction().commit();
		  sessionFactory.close();
	...
	public   List<AbstractCapabilityProposal> getProposalByAbstractCapability(AbstractCapability abstractCapability)
	{
		 SessionFactory sessionFactory = HibernateUtil.getSessionFactory();
		  Session session = sessionFactory.openSession();
		  session.beginTransaction();
		  Query query = session.createQuery("from AbstractCapabilityProposal where abstractCapability= :abstractCapability");
	...
	public void deleteAbstractCapabilityProposal(int idAbstractCapabilityProposal){
		 SessionFactory sessionFactory = HibernateUtil.getSessionFactory();
		  Session session = sessionFactory.openSession();
		  session.beginTransaction();
		  AbstractCapabilityProposal abstractCapabilityProposal = (AbstractCapabilityProposal)session.load(AbstractCapabilityProposal.class, idAbstractCapabilityProposal);
